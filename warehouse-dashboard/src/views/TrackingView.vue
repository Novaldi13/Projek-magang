<template>
  <div class="flex flex-col gap-6 w-full">
    <div class="flex justify-between items-center mb-2">
      <div>
        <h1 class="text-[24px] font-bold text-[#1e293b] dark:text-slate-100">
          Tracking Produk Terkirim
        </h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">
          Monitor pengiriman, garansi, dan riwayat perbaikan/reject produk
          Gitronik
        </p>
      </div>
      <Button
        label="Kirim Produk Baru"
        icon="pi pi-send"
        @click="openSendModal"
        class="shadow-sm"
      />
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <Card
        class="shadow-sm border-none rounded-xl dark:bg-slate-800"
        ...
        <template #content>
          <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
              <span
                class="text-xs font-semibold text-slate-500 dark:text-slate-400"
                >Total Semua Terkirim</span
              >
              <span
                class="text-2xl font-bold text-slate-800 dark:text-slate-100"
                >{{ stats.totalTerkirim }}</span
              >
            </div>
          </div>
        </template>
        ...
        <template #content>
          <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
              <span
                class="text-xs font-semibold text-slate-500 dark:text-slate-400"
                >Total Barang Reject</span
              >
              <span
                class="text-2xl font-bold text-slate-800 dark:text-slate-100"
                >{{ stats.totalReject }}</span
              >
            </div>
          </div>
        </template>
      ...
      >
        <template #content>
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-lg bg-amber-50 flex items-center justify-center text-amber-500"
            >
            ...
            <div class="flex flex-col">
              <span
                class="text-xs font-semibold text-slate-500 dark:text-slate-400"
                >Dalam Perbaikan</span
              >
              <span
                class="text-2xl font-bold text-slate-800 dark:text-slate-100"
                >{{ stats.totalPerbaikan }}</span
              >
            </div>
          </div>
        </template>
    ...
    <Card class="shadow-sm border-none rounded-xl dark:bg-slate-800">
      <template #content>
        <DataTable
          ...
          filterDisplay="menu"
          :filters="filters"
          :globalFilterFields="['sn', 'name', 'company.name']"
        ...
        >
            ...
            <div class="flex justify-between items-center mb-2">
              ...
              <span class="p-input-icon-left">
                ...
                <InputText
                  ...
                  placeholder="Cari SN / Produk / Company"
                />
              </span>
            </div>
          </template>
          ...
          <Column header="Produk">
            <template #body="slotProps">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 border border-slate-200 dark:border-slate-600 rounded overflow-hidden flex shrink-0 items-center justify-center bg-slate-50 dark:bg-slate-900 cursor-pointer hover:border-blue-400"
                  @click="viewImage(slotProps.data)"
                >
                  <img
                ...
                <div class="flex flex-col">
                  <span
                    class="font-bold text-slate-800 dark:text-slate-100 text-sm"
                    >{{ slotProps.data.sn }}</span
                  >
          ...
          <Column field="company.name" header="Company Tujuan">
            <template #body="slotProps">
              <div
                ...
                @click="viewCompany(slotProps.data.company)"
              >
            ...
            <template #body="slotProps">
              <div class="flex flex-col gap-1">
                <span class="text-xs text-slate-600 dark:text-slate-300"
                  ...
                  >{{ formatDate(slotProps.data.sendDate) }}</span
                >
                <div class="flex items-center gap-1 mt-1">
                  <Tag
                    :severity="getWarrantySeverity(slotProps.data).severity"
                  ...
                  <span
                    ...
                    >({{
                      getWarrantySeverity(slotProps.data).remainingText
                    }})</span
                  >
                </div>
              </div>
          ...
          <Column
            field="totalItems"
            header="Qty"
            class="text-center font-semibold text-slate-700 dark:text-slate-200"
          ></Column>
          ...
          <Column header="Aksi">
            <template #body="slotProps">
              <div class="flex gap-1">
                ...
                <Button
                  v-if="
                    slotProps.data.status === 'Terkirim' ||
                ...
                />
                <Button
                  v-if="slotProps.data.status.startsWith('Rejected')"
                  icon="pi pi-wrench"
                  text
                  rounded
                  severity="warning"
                  title="Proses Perbaikan"
                  @click="startPerbaikan(slotProps.data)"
                />
                <Button
                  v-if="slotProps.data.status === 'Perbaikan'"
                  icon="pi pi-check-circle"
                  text
                  rounded
                  severity="success"
                  title="Selesai Perbaikan"
                  @click="finishPerbaikan(slotProps.data)"
                />
              </div>
            </template>
          </Column>
          <Column field="status" header="Riwayat">
            <template #body="slotProps">
              <Button
                icon="pi pi-history"
                text
      ...
      <div class="flex flex-col gap-5 mt-2">
        <div class="grid grid-cols-2 gap-4">
          <div class="flex flex-col gap-2">
            <label
              ...
              >Kode SN (Serial Number)</label
            >
            <InputText
              v-model="newProduct.sn"
              placeholder="Contoh: SN-GIT-001"
            />
          </div>
          <div class="flex flex-col gap-2">
            <label
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-2">
            <label
              class="text-sm font-semibold text-slate-700 dark:text-slate-200"
              >Durasi Garansi (Bulan)</label
            >
            <InputNumber
              v-model="newProduct.warrantyMonths"
        ...
        <div
          class="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 flex flex-col gap-3"
        >
          <h3 class="font-semibold text-slate-700 dark:text-slate-200 text-sm">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-2">
              ...
              >
              <InputText
                v-model="newProduct.company.name"
                placeholder="PT. ABC"
              />
            </div>
            <div class="flex flex-col gap-2">
              ...
              >
              <InputText
                v-model="newProduct.company.pic"
            ...
            <div class="col-span-2 flex flex-col gap-2">
              ...
              >
              <InputText
                v-model="newProduct.company.address"
    ...
    >
      <div v-if="selectedProduct" class="flex flex-col gap-4 mt-2">
        <div
          class="bg-red-50 p-3 rounded border border-red-100 text-red-700 text-sm"
        >
          Anda akan me-reject barang <b>{{ selectedProduct.sn }}</b> dari
          perusahaan <b>{{ selectedProduct.company.name }}</b
          >. <br />Counter reject akan naik menjadi
          <b>Rejected {{ selectedProduct.rejectCounter + 1 }}</b
          >.
        </div>
        ...
        <div class="flex flex-col gap-2">
          <label
            ...
            >Catatan / Alasan Reject</label
          >
          <Textarea
            v-model="rejectNote"
            rows="4"
            placeholder="Contoh: Barang tidak menyala saat di-test..."
          />
        </div>
      </div>
      <template #footer>
    ...
    >
      <div v-if="selectedProduct" class="flex flex-col gap-4 mt-2">
        <div
          class="flex items-center justify-between bg-slate-50 dark:bg-slate-900 p-3 rounded border border-slate-200 dark:border-slate-600"
        >
          <div>
          ...
          <div class="text-right">
            ...
            <div class="font-bold text-red-500 text-lg">
              {{ selectedProduct.rejectCounter }}x
            </div>
          </div>
        </div>

        <div
          v-if="selectedProduct.rejectHistory.length === 0"
          class="text-center p-6 text-slate-400 italic"
        >
          Belum ada riwayat reject untuk produk ini.
        </div>
        <div v-else class="flex flex-col gap-3">
          <div
            v-for="(hist, idx) in selectedProduct.rejectHistory"
            :key="idx"
            class="border-l-2 border-red-400 pl-3 pb-2"
          >
            <div class="flex items-center gap-2 mb-1">
              <Tag
                severity="danger"
                :value="hist.statusName"
            ...
            <p
              class="text-sm text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 p-2 rounded"
            >
              {{ hist.note }}
            </p>
          </div>
    ...
    >
      <div v-if="selectedCompany" class="flex flex-col gap-4 mt-2">
        <div
          class="flex flex-col items-center justify-center p-4 bg-slate-50 dark:bg-slate-900 rounded-lg"
        >
          <div
            class="w-16 h-16 rounded-full bg-blue-100 text-blue-500 flex items-center justify-center text-2xl font-bold mb-3"
          >
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex justify-between border-b border-slate-100 py-2">
            ...
            >
            <span
              class="font-medium text-slate-800 dark:text-slate-100 text-sm text-right max-w-[60%]"
              >{{ selectedCompany.address }}</span
            >
          </div>
        </div>
      </div>
...
const products = ref<Product[]>([
  ...
  },
  {
    ...
    rejectHistory: [
      {
        date: "2026-04-12",
        statusName: "Rejected 1",
        note: "Layar sensor tidak merespon sentuhan setelah 2 minggu.",
      },
    ],
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1563770660941-20978e870e26?auto=format&fit=crop&q=80&w=200",
    ...
    rejectHistory: [
      ...
      {
        date: "2026-05-10",
        statusName: "Rejected 2",
        note: "Masih overheating setelah perbaikan pertama.",
      },
    ],
  },
]);
...
const stats = computed(() => {
  ...
  products.value.forEach((p) => {
    ...
    if (p.status === "Terkirim" || p.status === "Selesai")
      totalTerkirim += p.totalItems;
...
const getWarrantySeverity = (product: Product) => {
  ...
  if (diffDays <= 0) {
    ...
    return {
      label: "Aktif",
      severity: "success",
      remainingText: `${diffMonths} bln sisa`,
    };
  }
};
