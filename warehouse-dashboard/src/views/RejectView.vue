  ...
  <div class="flex flex-col gap-6 w-full">
            ...
            <Column field="pic.name" header="PIC Reject" sortable>
              <template #body="slotProps">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-slate-500 dark:text-slate-400">{{ slotProps.data.pic.role || 'No Role' }}</span>
                </div>
              </template>
            </Column>
    ...
    <Dialog :draggable="false" v-model:visible="showAddModal" modal header="Add Barang Reject" :style="{ width: '40vw' }" :breakpoints="{ '960px': '75vw', '641px': '90vw' }">
      <div class="flex flex-col gap-4 mt-2">
        ...
        <div class="flex flex-col gap-2">
          <label for="quantity" class="font-medium text-slate-700 dark:text-slate-200">Jumlah Barang</label>
          <InputNumber id="quantity" v-model="newItem.quantity" inputId="minmax-buttons" mode="decimal" showButtons :min="1" />
        </div>
        
        <div class="flex flex-col gap-2">
          <label for="rejectReason" class="font-medium text-slate-700 dark:text-slate-200">Status / Alasan Reject</label>
        ...
        <div class="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 flex flex-col gap-3">
          <p class="font-medium text-slate-700 dark:text-slate-200">PIC Reject (Penanggung Jawab)</p>
          <div class="flex flex-col gap-2">
             <label for="picName" class="text-sm text-slate-600 dark:text-slate-300">Nama PIC</label>
             <InputText id="picName" v-model="newItem.pic.name" placeholder="Masukkan nama PIC" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-2">
               <label for="picId" class="text-sm text-slate-600 dark:text-slate-300">ID User</label>
               <InputText id="picId" v-model="newItem.pic.id" placeholder="Masukkan ID User" />
            </div>
            <div class="flex flex-col gap-2">
               <label for="picRole" class="text-sm text-slate-600 dark:text-slate-300">Divisi / Role (Opsional)</label>
    ...
    <Dialog :draggable="false" v-model:visible="showProcessModal" modal :header="isViewingOnly ? 'Detail Barang Reject' : 'Proses ke Gudang (Submit)'" :style="{ width: '40vw' }" :breakpoints="{ '960px': '75vw', '641px': '90vw' }">
      <div v-if="selectedItem" class="flex flex-col gap-4 mt-2">
        <div class="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100">
          <div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-1">Nama Barang</p>
            <p class="font-medium text-slate-800 dark:text-slate-100">{{ selectedItem.itemName }}</p>
          </div>
          <div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-1">Jumlah</p>
            <p class="font-medium text-slate-800 dark:text-slate-100">{{ selectedItem.quantity }}</p>
          </div>
          <div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-1">Alasan Reject</p>
            <p class="font-medium text-slate-800 dark:text-slate-100">{{ selectedItem.rejectReason }}</p>
          </div>
          <div>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-1">Status Saat Ini</p>
        ...
        <div v-if="!isViewingOnly" class="flex flex-col gap-4 mt-2">
           <div class="flex flex-col gap-2">
             <label for="processDetail" class="font-medium text-slate-700 dark:text-slate-200">Detail Barang Reject</label>
             <Textarea id="processDetail" v-model="processData.details" rows="3" placeholder="Masukkan detail kondisi barang" />
           </div>
           <div class="flex flex-col gap-2">
             <label for="processNote" class="font-medium text-slate-700 dark:text-slate-200">Catatan Tambahan</label>
        ...
        <div v-else class="flex flex-col gap-4 mt-2">
           <div class="flex flex-col gap-1">
             <span class="font-medium text-slate-700 dark:text-slate-200 text-sm">Detail Barang Reject</span>
             <p class="text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 p-3 rounded border border-slate-100 whitespace-pre-wrap">{{ selectedItem.details || '-' }}</p>
           </div>
           <div class="flex flex-col gap-1">
             <span class="font-medium text-slate-700 dark:text-slate-200 text-sm">Catatan Tambahan</span>
             <p class="text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 p-3 rounded border border-slate-100 whitespace-pre-wrap">{{ selectedItem.notes || '-' }}</p>
           </div>
        </div>
      </div>
