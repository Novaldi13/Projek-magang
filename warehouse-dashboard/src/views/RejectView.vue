<template>
  <div class="flex flex-col gap-6 w-full">
    ...
    >
      <div>
        <h1
          class="text-[24px] font-bold text-[#1e293b] dark:text-slate-100 leading-tight"
        >
      ...
      >
        <template #content>
          <div class="flex items-center gap-4">
            <div
              :class="`w-12 h-12 rounded-lg flex items-center justify-center ${stat.bg}`"
            >
              <svg v-if="stat.label === 'No Submit'" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" :class="stat.color" class="lucide lucide-file-x-icon lucide-file-x"><path d="M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z"/><path d="M14 2v5a1 1 0 0 0 1 1h5"/><path d="m14.5 12.5-5 5"/><path d="m9.5 12.5 5 5"/></svg>
              <i v-else :class="`${stat.icon} text-xl ${stat.color}`"></i>
            </div>
            <div class="flex flex-col">
              <span
                class="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider"
                >{{ stat.label }}</span
              >
              <span
                class="text-2xl font-bold text-[#1e293b] dark:text-slate-100 leading-tight"
                >{{ stat.value }}</span
              >
            </div>
          </div>
    ...
    <!-- Data Table Section -->
    <Card
      class="shadow-sm border-none rounded-xl dark:bg-slate-800 overflow-hidden"
    >
      <template #content>
        <DataTable
          :value="rejects"
        ...
        >
          <template #header>
            <div
              class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-2"
            >
          ...
          <Column
            ...
            header="Batch ID"
            sortable
            class="font-mono text-xs text-blue-500 font-semibold"
          ></Column>
          ...
          <Column header="Informasi Barang" sortable field="itemName">
            <template #body="slotProps">
              <div class="flex items-center gap-3">
                <div
                  class="w-9 h-9 rounded-md bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400"
                >
                  <i class="pi pi-box"></i>
                </div>
                <div class="flex flex-col">
                  <span
                    class="font-bold text-slate-700 dark:text-slate-100 text-sm"
                    >{{ slotProps.data.itemName }}</span
                  >
          ...
          <Column
            ...
            header="Qty"
            sortable
            class="text-center font-bold text-slate-700 dark:text-slate-200"
          ></Column>
          ...
          <Column field="pic.name" header="PIC / Pelapor" sortable>
            <template #body="slotProps">
              <div class="flex items-center gap-2">
                <div
                  class="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-600 flex items-center justify-center text-[10px] font-bold text-slate-600 dark:text-slate-200"
                >
                  {{ slotProps.data.pic.name.charAt(0) }}
                </div>
                <div class="flex flex-col">
                  <span
                    class="text-xs font-medium text-slate-700 dark:text-slate-200 leading-tight"
                    >{{ slotProps.data.pic.name }}</span
                  >
                  <span class="text-[10px] text-slate-500">{{
                    slotProps.data.pic.role
                  }}</span>
                </div>
              </div>
            </template>
      ...
      <div class="flex flex-col gap-4 mt-2">
        <div class="flex flex-col gap-1.5">
          ...
          <InputText
            ...
            placeholder="Contoh: Motherboard X100"
            class="w-full"
          />
        </div>
        ...
        <div class="flex flex-col gap-1.5">
          ...
          >
          <InputText
            v-model="newItem.rejectReason"
            placeholder="Masukkan deskripsi alasan..."
            class="w-full"
          />
        </div>

        <div
          class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex flex-col gap-3"
        >
    ...
    >
      <div v-if="selectedItem" class="flex flex-col gap-5 mt-2">
        <!-- Item Overview Card -->
        <div
          class="bg-gradient-to-r from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm"
        >
          <div class="flex justify-between items-start mb-4">
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-500"
              >
                <i class="pi pi-box"></i>
              </div>
              <div class="flex flex-col">
                <span
                  class="text-xs font-mono text-blue-500 font-bold uppercase"
          ...
          <div class="grid grid-cols-2 gap-y-3 text-sm">
            <div class="flex flex-col">
              ...
              <span class="font-semibold text-slate-700 dark:text-slate-200"
                >{{ selectedItem.quantity }} Unit</span
              >
            </div>
            <div class="flex flex-col">
              <span
            ...
            <div
              class="flex flex-col col-span-2 border-t border-slate-100 dark:border-slate-700 pt-2 mt-1"
            >
              <span
                class="text-[10px] text-slate-400 uppercase font-bold tracking-wider"
              ...
              >
              <span class="font-semibold text-slate-700 dark:text-slate-200"
                >{{ selectedItem.pic.name }} ({{ selectedItem.pic.role }})</span
              >
            </div>
          </div>
        </div>
        ...
        <div v-if="!isViewingOnly" class="flex flex-col gap-4">
          <div class="flex flex-col gap-1.5">
            <label
              class="text-sm font-semibold text-slate-700 dark:text-slate-200"
              >Kondisi Detail Barang</label
            >
            <Textarea
              v-model="processData.details"
              rows="3"
              placeholder="Masukkan detail kerusakan..."
              class="text-sm"
            />
          </div>
          <div class="flex flex-col gap-1.5">
            <label
              class="text-sm font-semibold text-slate-700 dark:text-slate-200"
              >Instruksi / Catatan Gudang</label
            >
            <Textarea
              v-model="processData.notes"
              rows="2"
              placeholder="Contoh: Segera retur ke vendor..."
              class="text-sm"
            />
          </div>
        </div>
        ...
        <div v-else class="flex flex-col gap-4">
          <div
            class="flex flex-col gap-1.5 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700"
          >
            <span
            ...
            >
              {{ selectedItem.details || "Tidak ada detail kondisi." }}
            </p>
          </div>
          <div
            class="flex flex-col gap-1.5 p-3 rounded-lg bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30"
          >
            <span
              class="text-xs font-bold text-amber-500 uppercase tracking-widest"
              >Catatan / Instruksi</span
            >
            <p
              class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed"
            >
              {{ selectedItem.notes || "Tidak ada catatan." }}
            </p>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        ...
        <Button
          v-if="!isViewingOnly"
          label="Konfirmasi Masuk Gudang"
          severity="success"
          icon="pi pi-check-circle"
          @click="submitToGudang"
        />
      </template>
    </Dialog>
  </div>
...
const rejectStats = computed(() => {
  ...
  const noSubmitCount = rejects.value.filter(
    (r) => r.status === "No Submit",
  ).length;
  const submitCount = rejects.value.filter((r) => r.status === "Submit").length;

  return [
    {
      label: "Total Unit Reject",
      value: total,
...
const saveRejectItem = () => {
  if (
    ...
    !newItem.value.rejectReason ||
    !newItem.value.pic.name
  )
    return;
